...
from .GRN_Models.A_MRNA_Single_UpRegulation import transitions as trans_Up_G1
from .GRN_Models.B_MRNA_Single_UpRegulation import transitions as trans_Up_G2
...
from .GRN_Models.A_MRNA_Single_UpRegulation import propensities as prop_Up_G1 # extract M1, M2
from .GRN_Models.B_MRNA_Single_UpRegulation import propensities as prop_Up_G2 # extract M1, M2
...
def generate_data_twoGRN(var_data = False, noise = False):
    ...
    data_dic = {}
    class_type1 = ["NoI", "Up_G1", "Up_G2", "Up_G1G2"] # bivariate Normal Dist
    ssa_func_list = [ssa_func, ssa_func, ssa_func, ssa_func] # can exclude change of parameters in this form
    trans = [(trans_NoI_G1, trans_NoI_G2), (trans_Up_G1, ), (trans_Up_G2, ), (trans_Up_G1G2, )]
    propens = [(prop_NoI_G1, prop_NoI_G2), (prop_Up_G1, ), (prop_Up_G2, ), (prop_Up_G1G2, )]
    ...
    for i in range(len(class_type1)):
        ...
        for j in range(MaxNumVar + 1):
            if j < num_var[lab[0]]:
                ...
                class_dic[class_type1[i]+"%d_%d"%(j+1, 0)] = lab
                class_dic[class_type1[i]+"%d_%d"%(j+1, 1)] = lab
        k += 1    
