...
for j in range(1):#len(var_data_list)):
    ...
    if dic_meth["class_method"] == "MIASA":
        acc_list, adjusted_acc_list, num_it_list = repeated_classifications(repeat, method_dic_list, generate_data = generate_data, c_dic = c_dic, var_data = var_data_list[j], n_jobs = 8, plot = plotfew, in_threads = in_threads, separation = sep_vars)    
        file = open(save_at + "Num_it_set_%d_%d_varS%s.pck"%(set_num, repeat, var_data_list_labs[j]), "wb")
        pickle.dump({"method_name":method_name, "method_list":method_dic_list, "accuracy_list":acc_list, "num_iterations": num_it_list}, file)
        file.close()
    else:
